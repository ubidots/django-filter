...
__all__ = [
    'FilterGroup', 'ExclusiveGroup', 'RequiredGroup', 'CombinedGroup',
    'CombinedRequiredGroup',
]


class FilterGroup:
    """Validate & filter query parameters as a group.

    Attributes:
        filters: The set of filter names to operate on.
        parent: The parent ``FilterSet`` instance.
    ...
    def validate(self, form, **data):
        ...
        Args:
            ...
            **data: The subset of a form's ``cleaned_data`` for the filters in
                the group. This is a result of ``self.extract_data``.
        """
    ...
    def format_labels(self, filters):
        ...
        and combines them into a formatted string.
    ...
    def _filter_data(self, data):
        ...
        return {k: v for k, v in data.items() if k not in EMPTY_VALUES}


class ExclusiveGroup(FilterGroup):
...
class RequiredGroup(FilterGroup):
    """A group of mutually required filters.

    If any filter in the group is included in the request data, then all other
    filters in the group **must** be present in the request data.

    Filtering is performed by the individual filters and is not combined via
    ``Q`` objects.
    """
...
class CombinedGroup(FilterGroup):
    """A group of filters that result in a combined query.
